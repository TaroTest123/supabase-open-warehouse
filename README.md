...
│          ▲                                              │
│  ┌───────┴───────┐                                      │
│  │  TEPCO CSV/ZIP│ ← でんき予報データ（日次CSV+月次ZIP）    │
│  │  (データ取込)  │                                      │
│  └───────────────┘                                      │
└─────────────────────────────────────────────────────────┘
```
...
- [TEPCO CSV データ](https://www.tepco.co.jp/forecast/html/images/juyo-d-j.csv) — 当日実績 CSV（1時間間隔）
- TEPCO 月別 ZIP — `YYYYMM_power_usage.zip`（5分間隔、供給力・使用率含む）
