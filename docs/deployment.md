...
> `<secret_key>` には `service_role` key（`eyJ...` 形式の JWT）を使用します。
...
| `SUPABASE_SERVICE_ROLE_KEY` | service_role key | `eyJ...`（JWT 形式） |
...
| `SUPABASE_SERVICE_ROLE_KEY` | Edge Function 認証 | Settings → API Keys → `service_role` key（`eyJ...` 形式の JWT） |
...
- `service_role` key（`eyJ...` 形式の JWT）を使用しているか確認（`anon` key ではない）
