...
| `SUPABASE_DB_HOST` | dbt から DB に接続 | Connect → Connection pooler のホスト名（例: `aws-0-ap-northeast-1.pooler.supabase.com`） |
| `SUPABASE_DB_PORT` | dbt DB ポート | Connection pooler のポート（`6543`） |
...
| `deploy-supabase.yml` | `production` | `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF` | main push (`supabase/migrations/**` or `supabase/functions/**` 変更時) / 手動 |
