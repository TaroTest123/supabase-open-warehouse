...
## Addendum: ZIP 対応（2026-03-13 追記）

TEPCO は 2022年4月以降、年次 CSV の更新を停止し、月別 ZIP (`YYYYMM_power_usage.zip`) で5分間隔データを公開する形式に移行した。これに対応するため以下を追加:

- **Edge Function**: URL が `.zip` で終わる場合、`fflate` で展開し各 CSV をパースする分岐を追加
